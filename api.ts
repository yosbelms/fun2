import { deepMap, getProp, isFunction, genericMiddleware } from './util'
...
export const callInApi = (
  ...
  args: any[] = [],
  ctx?: any,
  middleware?: (ctx: any, next: Function) => any
) => {
  const prop = getProp(api, basePath)
  const next = () => {
    if (isFunction(prop[method])) return prop[method](...args)
    throw new TypeError(`${basePath.join('.')}.${method} is not a function`)
  }
  if (isFunction(middleware)) {
    return (middleware as Function)(ctx, next)
  } else {
    return genericMiddleware(ctx, next)
  }
}
